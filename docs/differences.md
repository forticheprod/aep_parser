...
`numLayers`, `numItems`, `numProperties`, and `numOutputModules` are not needed
since `len()` works on Python lists:

```python
len(comp.layers)       # equivalent to comp.numLayers
len(project.items)     # equivalent to project.numItems
len(group.properties)  # equivalent to group.numProperties
```
...
`CompItem` provides typed, filtered layer lists:
