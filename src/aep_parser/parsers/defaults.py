...
def _fill_group(
    ...
    for spec in specs:
        if spec.match_name in existing:
            ...
            if isinstance(child, Property):
                child.is_spatial = spec.is_spatial
                child.color = spec.color
                if spec.min_value is not None:
                    child.min_value = spec.min_value
                if spec.max_value is not None:
                    child.max_value = spec.max_value
                if spec.can_vary_over_time is not None:
                    child.can_vary_over_time = spec.can_vary_over_time
                if child.default_value is None and child.value is None:
                    dv = (
                        spec.value
