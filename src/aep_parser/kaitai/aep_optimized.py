...
def _build_chunk_type_mapping() -> tuple[dict[str, type], type]:
    ...
    # Pattern to match: if/elif _on == u"chunk_type": self.data = Aep.ClassName(
    # Handles both: if _on == u"sspc": and elif _on == u"tdmn":
    pattern = re.compile(
        r'(?:if|elif) _on == u?"([^"]+)":\s*\n\s*self\.data = Aep\.(\w+)\(',
        re.MULTILINE
    )
...
def _optimized_chunk_read_no_fallback(self: Aep.Chunk) -> None:
    """Optimized _read method for Chunk using dict lookup instead of if/elif."""
    self.chunk_type = (self._io.read_bytes(4)).decode("ascii")
    self.len_data = self._io.read_u4be()
    self._raw_data = self._io.read_bytes(
...
def apply_optimizations() -> None:
    ...
    """
    if _FALLBACK_CLASS is None:
        Aep.Chunk._read = _optimized_chunk_read_no_fallback
    else:
