...
class TestMultiFileDifference:
    ...
    def test_multiple_bits_no_position(self) -> None:
        ...
        diff = MultiFileDifference(
            path="test", offset=0, values=[0x00, 0xFF, 0x00]
        )
        assert diff.bit_position is None
    ...
    def test_three_distinct_values_no_position(self) -> None:
        ...
        diff = MultiFileDifference(
            path="test", offset=0, values=[0x00, 0x01, 0x02]
        )
        assert diff.bit_position is None
            ...
            for j, p2 in enumerate(paths):
                if i != j:
                    assert not p2.startswith(p1 + "/"), (
                        f"Parent chunk {p1} has diffs that should only "
                        f"be in child {p2}"
                    )
...
class TestToJsonOutput:
    ...
    def test_json_output_structure(self) -> None:
        ...
        diff = ChunkDifference(
            path="test/path",
            byte_diffs=[
                ByteDifference(path="test", offset=0, byte1=0x00, byte2=0x01)
            ],
            size1=10,
            size2=10,
        )
